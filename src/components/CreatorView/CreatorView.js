...
export default function CreatorView({ theQuiz, userList, socket, room }) {
  ...
  const startQuiz = () => {
    ...
    fadeAnswers();
    const startObj = {
      msg: "Quiz has started",
      status: true,
      room,
      question: theQuiz[currentQuestion],
      questionCount: theQuiz.length,
    };
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < theQuiz.length - 1) {
      ...
      const questionObj = {
        ...
        status: true,
        room,
        question: theQuiz[oldQuestion + 1],
        currentIndex: oldQuestion + 1,
      };
  ...
  const answerClasses = show
    ? "quizAnswers answerBorder"
    : "quizAnswers answerBorder collapsed";
  return (
      ...
      {finalAnswer ? (
        ...
        <>
          <div className="leftSidebar">
            ...
            <div className="quizProgress">
              {currentQuestion + 1}/{theQuiz.length}
            </div>
          </div>
          <div className="theQuiz">
            <div className="quizQuestion">
              {theQuiz[currentQuestion].question}
            </div>
            <div className={answerClasses}>
              {theQuiz[currentQuestion].answers.map((item) => (
                <div key={item.id} className={itemClass}>
                  {item.answer}
                </div>
              ))}
            </div>
            <button className="quizButton" onClick={() => nextQuestion()}>
        ...
        <>
          ...
          <div className="theQuiz">
            ...
            <button onClick={() => showPrompt()}>Start Quiz</button>
          </div>
          {startModal && (
            <StartModal startQuiz={startQuiz} setStartModal={setStartModal} />
          )}
        </>
      )}
      <UserList
        users={userList}
        answeredUsers={usersWhoHaveAnswered}
        socket={socket}
      />
    </div>
  );
